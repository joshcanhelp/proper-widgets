...
class PhpFormBuilder {
	...
	function build_form( $echo = true ) {
		...
		foreach ( $this->inputs as $key => $val ) :
			...
			// TODO: Not doing this right for blank values
			...
			switch ( $val['type'] ) :
				...
				case 'select':
					$element = 'select';
					$end     = '>' . $this->_output_options_select( $val['options'], $val['selected'] ) . '
					</select>';
					break;
	...
	private function _output_options_select( $arr, $selected = FALSE ) {
		$output = '';
		foreach ( $arr as $val => $opt ) :
			...
			$output .= '
						<option value="' . $val . '"' . $opt_insert . '>' . $opt . '</option>';
		endforeach;
		return $output;
	}
